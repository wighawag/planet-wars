...
async function setupContracts(address) {
  ...
  if (_chainConfigs) {
    ...
    for (const contractName of Object.keys(contractsInfos)) {
      ...
      contractsToAdd[contractName] = new Contract(contractInfo.address, contractInfo.abi, _ethersProvider.getSigner(address));
    }
  }
  set({contracts: {
    status: 'Ready',
...
async function select(type) {
  ...
  if (type === 'builtin') {
    ...
    // TODO load module _ethersProvider =nnew Web3Provider(web3Provider); 
  }
...
function probeBuiltin(config = {}) {
  ...
  probing = new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (ethereum) {
        _builtinProvider = new Web3Provider(ethereum);
        ...
        if (config.metamaskReloadFix && $wallet.builtin.vendor === "Metamask") {
          ...
          await timeout(200, _builtinProvider.send("eth_chainId", []), () => {
